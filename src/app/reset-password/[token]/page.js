...
export default function ResetPasswordPage({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setApiError(err.message || "Link is invalid or expired. Please request another reset link.");
    } finally {
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {apiSuccess && (
                  <div className="p-4 text-sm text-green-700 bg-green-50 border border-green-200 rounded-xl flex items-start gap-2.5">
                    ...
                    <span className="font-semibold">{apiSuccess}</span>
                  </div>
                )}
      ...
      <div className="relative flex-1 hidden w-0 lg:block bg-blue-600 overflow-hidden">
        ...
        <div className="relative flex flex-col justify-center h-full max-w-xl mx-auto px-12 text-white">
          ...
          <h1 className="text-5xl font-extrabold tracking-tight leading-none mb-6">
            Keep your dispatch pipelines secure.
          </h1>
          <p className="text-lg text-blue-100 leading-relaxed">
            Ensure you choose a strong password containing numbers, letters, and special characters. Do not share your login credentials with anyone.
          </p>
        </div>
      </div>
    </div>
